...
def pick(paragraphs, select, k):
    ...
    if len(filtered) <= 0 or len(filtered) <= k:
        return ''
    else:
        return filtered[k]
    # END PROBLEM 1
...
def about(subject):
    ...
    def select(paragraph):
        filtered = split(remove_punctuation(lower(paragraph)))
        for subj in subject:
...
def accuracy(typed, source):
    ...
    # BEGIN PROBLEM 3
    real = len(typed_words)
    needed = len(source_words)
    if real == needed == 0:
        return 100.0
    k = needed if needed < real else real
    count = 0
    for i in range(k):
        if typed_words[i] == source_words[i]:
            count += 1
    return count / (real / 100) if count != 0 else 0.0
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if typed_word in word_list:
        return typed_word
    lst = [diff_function(typed_word, word, limit) for word in word_list]
    num = min(lst)
    ind = lst.index(num)
    if lst[ind] <= limit:
...
def time_per_word(words, timestamps_per_player):
    ...
    # BEGIN PROBLEM 9
    time = []
    for times in timestamps_per_player:
        player_times = []
        for i in range(1, len(times)):
            player_times.append(times[i]-times[i-1])
        time.append(player_times)
...
def fastest_words(match):
    ...
    for word in word_indices:
        word_times = [time(match, i, word) for i in player_indices]
        min_t = min(word_times)
        ind = word_times.index(min_t)
        fastest[ind].append(get_word(match, word))
    return fastest
    # END PROBLEM 10
