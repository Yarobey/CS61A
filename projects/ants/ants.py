...
class Ant(Insect):
    ...
    def double(self):
        ...
        # BEGIN Problem 12
        if not self.was_doubled:
            self.damage *= 2
            self.was_doubled = True
        if isinstance(self, ContainerAnt):
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        """Eat random whole bee alive or chew it, if it was eaten less than 3 turns ago"""
...
class TankAnt(ContainerAnt):
    """TankAnt provides protection to other Ants and damages nearest bees."""
...
class ScubaThrower(ThrowerAnt):
    """ ScubaThrower is a ThrowerAnt that finally can swim."""
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        # BEGIN Problem Optional 2
        insects_dict, current_place, distance = {}, self.place, 0
        while current_place.is_hive == False:
            if (current_place.ant is not self) and (current_place.ant is not None):
