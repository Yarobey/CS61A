...
def interleaved_sum(n, odd_term, even_term):
    ...
    True
    """
    def helper(func1, func2, k):
        if k == n:
            return func1(k)
        return helper(func2, func1, k+1) + func1(k)
...
def count_coins(total):
    ...
    True
    """
    def count_coins_helper(total_left, coin):
        if total_left == 0:
            return 1
        elif total_left < 0:
            return 0
        if coin == None:
            return 0
        without_coin = count_coins_helper(total_left, next_smaller_coin(coin))
        with_coin = count_coins_helper(total_left - coin, coin)
...
def move_stack(n, start, end):
    ...
    def find_intermediate(start, end):
        if start == 3 or end == 3:
